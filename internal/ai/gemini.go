...
func (p *GeminiProvider) SimpleGenerate(ctx context.Context, prompt string) (string, *LLMUsage, error) {
	if p == nil || p.apiKey == "" {
	...
	usage := geminiUsageFromResponse(resp, p.modelName)
...
func geminiUsageFromResponse(resp map[string]any, modelName string) *LLMUsage {
	in, out := 0, 0
	...
	return &LLMUsage{Provider: "gemini", Model: modelName, InputTokens: in, OutputTokens: out}
}
	...
	for iter := 0; iter < maxToolCallIterations; iter++ {
		...
		if len(funcCalls) == 0 {
			...
			return GenerateResult{
				...
				Voice:        req.Voice,
				Usage: &LLMUsage{
					Provider: "gemini", Model: p.modelName,
