...
Modals.AutoRoutingConfig = (() => {
    ...
    function defaultClassifierProvider() {
        if (isLocalAIAllowedForChat()) return DEFAULT_CLASSIFIER_PROVIDER;
        const keys = AIModels.cached().map((m) => m.key);
        return keys[0] || DEFAULT_CLASSIFIER_PROVIDER;
    }
    ...
    function normalizeClassifierProvider(value) {
        ...
        if (key === 'localai' || AIModels.isValidKey(key)) return key;
        return defaultClassifierProvider();
    }
    ...
    function reconcileClassifierProvider() {
        ...
        if (typeof Modals !== 'undefined'
            ...
            && Modals.AIModelsConfig.reflectClassifierProvider) {
            Modals.AIModelsConfig.reflectClassifierProvider(resolved);
        }
    }
...
Modals.UserLLMSettings = (() => {
    ...
    /**
     * Colors a key field green when configured; otherwise reddish ('danger', for the
     * required OpenRouter key) or yellow ('warn', the default, for the optional keys).
     * Updates the adjacent status icon when present.
     */
    function applyLLMInputState(el, isOk, notOkSeverity) {
        if (!el) return;
        const severity = notOkSeverity === 'danger' ? 'danger' : 'warn';
        el.classList.remove(
            'user-llm-field-input--state-ok',
            'user-llm-field-input--state-warn',
            'user-llm-field-input--state-danger'
        );
        el.classList.add(isOk ? 'user-llm-field-input--state-ok' : `user-llm-field-input--state-${severity}`);

        const statusEl = el.id ? getEl(`${el.id}-status`) : null;
        if (!statusEl) return;
        statusEl.classList.remove('user-llm-key-status-icon--ok', 'user-llm-key-status-icon--warn', 'user-llm-key-status-icon--danger');
        if (isOk) {
            statusEl.classList.add('user-llm-key-status-icon--ok');
            statusEl.innerHTML = '<i class="fas fa-check-circle" aria-hidden="true"></i>';
            statusEl.setAttribute('aria-label', 'API key configured');
            statusEl.title = 'API key configured';
        } else {
            statusEl.classList.add(severity === 'danger' ? 'user-llm-key-status-icon--danger' : 'user-llm-key-status-icon--warn');
            statusEl.innerHTML = '<i class="fas fa-exclamation-triangle" aria-hidden="true"></i>';
            statusEl.setAttribute('aria-label', 'API key not configured');
            statusEl.title = 'API key not configured';
        }
    }

    function bindLLMKeyStatusInputRefresh(input) {
        if (!input || input.dataset.llmKeyStatusWired === '1') return;
        input.dataset.llmKeyStatusWired = '1';
        input.addEventListener('input', () => {
            const configured = input.dataset.llmKeyConfigured === '1';
            const severity = input.dataset.llmKeySeverity || 'warn';
            const ok = input.value.trim() !== '' || configured;
            applyLLMInputState(input, ok, severity);
        });
    }

    /**
     * Empty API key field: saved user key is never returned from the server — show a mask plus “(user supplied)”.
     * If the service is still usable (archive owner key on visitor sessions, or an env-configured
     * key for services that still support one, e.g. ElevenLabs), prompt to paste an override.
    ...
    function apiKeyPlaceholder(userKeySet, serviceConfiguredForUser, serviceNameForEmptyPrompt) {
        ...
        if (serviceConfiguredForUser) {
            return 'A key is already configured. Paste your own key to overwrite';
        }
    ...
    function refreshProviderAvailabilityAfterKeysChange() {
        if (typeof App !== 'undefined' && App.refreshChatAvailability) {
            void App.refreshChatAvailability();
        }
    }
    ...
    async function load(options = {}) {
        ...
        try {
            ...
            const runpodWorkersEnv = parseInt(av.runpod_workers_env, 10) || 0;
            ...
            if (intro) {
                if (sessionScoped) {
                    intro.textContent = 'Optional overrides for this visit only. Values are stored on your session and cleared when you sign out or the session expires. Leave a field blank to use the archive owner’s saved key where available.';
                } else {
                    intro.textContent = 'An OpenRouter API key is required to access cloud hosted AIs and hosted tools like web search and speech generation. Digital Museum will work fine without these services, but the responses may not be as sophisticated or as complete. Keys are configured only here — there is no server default — and are not shown again after saving. Leave a key field blank to keep your current saved key, or use Clear to remove it.';
                }
            }
            ...
            if (subEl) {
                if (sessionScoped) {
                    const parts = [];
                    if (llm.subject_openrouter_api_key_set) parts.push('OpenRouter API key');
                    ...
                    subEl.textContent = parts.length
                        ? ('When you leave a field blank, the owner’s saved settings apply where available. Owner has: ' + parts.join('; ') + '.')
                        : 'The archive owner has not saved personal API keys in Configuration — blank fields will not work until either the owner or you provide a key.';
            ...
            setHint('user-llm-openrouter-key-hint', !!llm.openrouter_api_key_set, sessionScoped);
            ...
            const ownerOpenRouterKey = !!(sessionScoped && llm.subject_openrouter_api_key_set);
            ...
            const openRouterKeyOk = !!(llm.openrouter_api_key_set || ownerOpenRouterKey);
            const tavKeyOk = !!(llm.tavily_api_key_set || ownerTavilyKey);
            const runpodKeyOk = !!(llm.runpod_api_key_set || ownerRunpodKey);
            const elevenlabsKeyOk = !!(llm.elevenlabs_api_key_set || ownerElevenlabsKey || elevenlabsEnv);
            const ork = getEl('user-llm-openrouter-key');
            const tk = getEl('user-llm-tavily-key');
            const rk = getEl('user-llm-runpod-key');
            const el11 = getEl('user-llm-elevenlabs-key');
            const openRouterKeyPh = apiKeyPlaceholder(!!llm.openrouter_api_key_set, openRouterKeyOk, 'OpenRouter');
            ...
            if (ork) {
                ork.value = '';
                ork.placeholder = openRouterKeyPh;
            }
            if (tk) {
                tk.value = '';
            ...
            const rpEpEl = getEl('user-llm-runpod-endpoint');
            const rpWkEl = getEl('user-llm-runpod-workers');
            const storedEndpoint = String(llm.runpod_endpoint_id || (sessionScoped ? llm.subject_runpod_endpoint_id : '') || '').trim();
            const storedWorkers = parseInt(llm.runpod_workers || (sessionScoped ? llm.subject_runpod_workers : 0), 10) || 0;
            if (rpEpEl) {
                rpEpEl.value = storedEndpoint;
                rpEpEl.placeholder = runpodEndpointEnv ? `Server: ${runpodEndpointEnv}` : 'Enter endpoint ID';
            }
            if (rpWkEl) {
                rpWkEl.value = String(storedWorkers > 0 ? storedWorkers : 0);
                if (storedWorkers === 0) {
                    rpWkEl.options[0].text = runpodWorkersEnv > 0
                        ? `Server default (${runpodWorkersEnv})`
                        : 'Server default';
                }
            }
            if (el11) {
            ...
            setLLMInfoButton('user-llm-info-openrouter-key', 'OpenRouter API key', openRouterKeyPh);
            ...
            wireLLMInfoPopover();
            applyLLMInputState(ork, openRouterKeyOk, 'danger');
            applyLLMInputState(tk, tavKeyOk, 'warn');
            applyLLMInputState(rk, runpodKeyOk, 'warn');
            applyLLMInputState(el11, elevenlabsKeyOk, 'warn');
            if (ork) {
                ork.dataset.llmKeyConfigured = openRouterKeyOk ? '1' : '';
                ork.dataset.llmKeySeverity = 'danger';
            }
            if (tk) {
                tk.dataset.llmKeyConfigured = tavKeyOk ? '1' : '';
                tk.dataset.llmKeySeverity = 'warn';
            }
            if (rk) {
                rk.dataset.llmKeyConfigured = runpodKeyOk ? '1' : '';
                rk.dataset.llmKeySeverity = 'warn';
            }
            if (el11) {
                el11.dataset.llmKeyConfigured = elevenlabsKeyOk ? '1' : '';
                el11.dataset.llmKeySeverity = 'warn';
            }
            bindLLMKeyStatusInputRefresh(ork);
            bindLLMKeyStatusInputRefresh(tk);
            bindLLMKeyStatusInputRefresh(rk);
            bindLLMKeyStatusInputRefresh(el11);
    ...
    async function save() {
        ...
        const body = {};
        const ork = (getEl('user-llm-openrouter-key') && getEl('user-llm-openrouter-key').value) || '';
        ...
        if (ork.trim()) body.openrouter_api_key = ork.trim();
        ...
        const rpEpVal = ((getEl('user-llm-runpod-endpoint') && getEl('user-llm-runpod-endpoint').value) || '').trim();
        body.runpod_endpoint_id = rpEpVal;
        const rpWkVal = parseInt((getEl('user-llm-runpod-workers') && getEl('user-llm-runpod-workers').value) || '0', 10);
        body.runpod_workers = isNaN(rpWkVal) ? 0 : rpWkVal;
        try {
            await patchLLM(body);
            ...
            refreshProviderAvailabilityAfterKeysChange();
        } catch (e) {
    ...
    async function clearLLMPatch(body) {
        ...
        try {
            ...
            setStatusThenFade('Cleared. Those overrides were removed; empty fields fall back to the owner’s saved key where available.', '#15803d', 12000);
        } catch (e) {
            setStatus(e.message || 'Clear failed', '#c00');
        }
    }

    const KEY_VISIBILITY_TOGGLE_IDS = ['openrouter-key', 'tavily-key', 'runpod-key', 'elevenlabs-key'];

    function initKeyVisibilityToggles() {
        KEY_VISIBILITY_TOGGLE_IDS.forEach((suffix) => {
            const input = getEl(`user-llm-${suffix}`);
            const btn = getEl(`user-llm-toggle-${suffix}`);
            if (!input || !btn) return;
    ...
    function init() {
        ...
        const saveBtn = getEl('user-llm-save-btn');
        if (saveBtn) saveBtn.addEventListener('click', () => void save());
        const cor = getEl('user-llm-clear-openrouter');
        if (cor) cor.addEventListener('click', () => void clearLLMPatch({ openrouter_api_key: '' }));
        ...
        if (crp) crp.addEventListener('click', () => void clearLLMPatch({ runpod_api_key: '', runpod_endpoint_id: '', runpod_workers: 0 }));
